...
$\Lambda$ is a subset of the real numbers, $\Lambda \subseteq \mathbb{R}$, and it specifies the set of wavelengths in nanometers that the LIBS data captures.
...
% Given a set \(E\) of eight major oxides detectable using LIBS, \(E = \{SiO_2, TiO_2, Al_2 O_3, FeOT, MgO, CaO, Na_2O, K_2O\}\), and considering \(\mathcal{P}(E)\) as the power set of \(E\) which represents all possible combinations of these oxides, we define \(K \subseteq \mathcal{P}(E) \) as the set of oxide compositions detected by LIBS.

% Let $S$ be the set of all possible spectra, where each spectrum $\mathbf{s} \in S$ is a multidimensional vector in $\mathbb{R}^n$ with $n$ being the number of wavelengths captured by the LIBS spectrometer for a single laser pulse. $\mathbf{s}$ holds the intensities of light emitted from the plasma that is created from the laser ablation process.
