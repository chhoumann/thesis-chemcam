...
An issue with LIBS data is that the spectra are often noisy and contain outliers.
This is due to the nature of the LIBS technique, which is sensitive to the surface conditions of the sample, such as dust and weathering layers.
In addition, the LIBS technique is sensitive to the distance between the instrument and the sample, which can vary depending on the terrain and the rover's position.
This can lead to spectral interference, which can affect the accuracy of the model's predictions.
Spectral interference occurs when the spectra of two or more elements overlap, making it difficult to distinguish between them.

The ChemCam team uses a model called the Multivariate Oxide Composition (MOC) model to predict the composition of major oxides in geological samples. As part of their preprocessing, they use various techniques to remove noise and outliers from the data.
