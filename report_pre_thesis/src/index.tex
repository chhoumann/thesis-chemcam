...
\section{Definition}
Given a set $E$ of elements detectable using LIBS, and considering \(\mathbb{P}(E)\) as the power set of $E$ which represents all possible combinations of these elements, we define \(K \subseteq \mathbb{P}(E) \) as the set of compositions of these elements that can be subjected to LIBS. Additionally, let $S$ be the set of all possible spectra emitted from these compositions.
...
\begin{definition}
    A prediction set $\mathbf{\hat{y}}$ is the weigthed average of the predictions from the PLS and ICA models defined by:

    \( \mathbf{\hat{y}} = w_{PLS} \times Y_{PLS} + w_{ICA} \times Y_{ICA} \)

    Let:
    \begin{itemize}
        \item $Y_{PLS}$ and $Y_{ICA}$ be the prediction sets from the PLS and ICA models, respectively.
        \item $w_{PLS}$ and $w_{ICA}$ be the weights assigned to the PLS and ICA models, respectively, such that \(w_{PLS} + w_{ICA} = 1 \), ensuring a weighted average.
    \end{itemize}
