...
\begin{definition}
    A prediction set \( \mathbf{\hat{y}} = w_PLS * Y_PLS + w_ICA * Y_ICA \) where \mathbf{\hat{y}} is the predicted value of the weighted average, $w_PLS$ is the weight assigned to the PLS model, $Y_PLS$ is the prediction from the PLS model, $w_ICA$ is the weight assigned to the ICA model and $Y_ICA$ is the prediction from the ICA model.
\end{definition}
