...
We present our XGBoost parameters in Section~\ref{sec:XGBoost_parameters}.
...
In spirit of the MOC pipeline, one could combine the predictions of the XGBoost and PLS1-SM (original) models to produce a hybrid model that leverages the strengths of both models. The original PLS1-SM model achieves superior performance on \ce{SiO2} and \ce{FeOT}, while the XGBoost model performs better on the other oxides. The hybrid model could be constructed by taking a weighted average of the predictions from the two models on those oxides.
