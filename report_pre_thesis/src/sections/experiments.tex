...
Furthermore, we found that the XGBoost model performs exceptionally well, only being outperformed by the original MOC model on \ce{FeO_T}.
...
% - Outlier detection is a less-limiting factor for PLS1-SM than for ICA, as PLS1-SM is (seemingly) more robust to outliers than ICA.
% - Using ICA seems to less beneficial than using PLS1-SM.
% - But PLS1-SM is analogous to ensemble models. However, the XGBoost model is also an ensemble model, and it performs better than PLS1-SM on most oxides.
% - The ANN model performs well, but it requires a large amount of training data to perform well. We don't have enough training data to train a good ANN model, so we can't draw any conclusions about its fully realized performance.
% - In the future, using ICA is likely not necessary (high RMSEs on both originals and replications, etc.). Perhaps using some form of dimensionality reduction could be beneficial, but ICA is not the right choice. This is evident from the fact that all other models outperform the ICA model on most oxides. Another good clue is that the SuperCam model evaluation (seen in paper "andersonPostlandingMajorElement2022") didn't even consider ICA. The point about Dimensionality reduction is also supported by PLS having it's own 'inbuilt' dimensionality reduction.
% - The automated outlier removal process in PLS1-SM is not necessary, as it does not have a significant impact on the model's performance - given that the data is preprocessed correctly. We found that it can become necessary when location data is aggregated, but that's only for ICA. We are unclear about whether manual outlier removal would improve results.
% - It's likely that the preprocessing steps are the least limiting factors in the MOC pipeline. The most limiting factors are likely the choice of model and the amount of training data available.

% Outlier removal - ingen effekt.
% Preprocessing - stor effekt.
% Model - stor effekt.

% -> Data er veludformet. Derfor er preprocessing af high-dim, LIBS data vigtigt. Derudover skal du have en model som kan fange kompleksiteten i dataen.

% - However, we did not have access to any data pre-RDR, so we can't draw any conclusions about the impact of the prior data processing steps. We found that RDR data is not sufficient to train a good model, so it's likely that the prior data processing steps are necessary. Masking and normalization seems to be the most important preprocessing steps.

The choice of model and the volume of training data are paramount in influencing the pipeline's accuracy and reliability.
...
Additionally, while Artificial Neural Networks (ANNs) demonstrate potential, their performance is critically dependent on the availability of substantial training data --- a resource presently insufficient for our purposes, thereby limiting our ability to fully assess ANNs' capabilities.
The anticipated utility of ICA is reconsidered in light of its high Root Mean Square Errors (RMSEs) and its underperformance relative to other models, suggesting a pivot away from ICA in future iterations of the pipeline. % todo: rephrase
Moreover, our investigation reveals that preprocessing steps, particularly automated outlier removal, do not critically impact model performance, indicating that the key to advancing the MOC pipeline lies not within preprocessing but in judicious model selection and the enhancement of the training data corpus.
...
This is further reinforced by \cite{andersonPostlandingMajorElement2022} also did model selection for the SuperCam instrument, opting not to use the MOC pipeline.
