...
\subsubsection{PLS-SM Regression}
% Describe full model into submodel
% Describe filter data by composition ranges (wt% ranges) - Also the adjustments made
% Load all models (full, low, mid, high) for each oxide
% Make full model prediction
% Using blending configuration, calculate weights and then return weighted blend
As mentioned in the \ref{sec:introduction} the PLS aspect of the pipeline utilizes a submodel approach to make oxide predictions.
It is structured such that each of the eight oxides in consideration will have either three or four, corresponding PLS models.
The core concept being that a PLS model, referred to as the \textit{full model}, will be responsible for making an initial prediction for each oxide and in this case, eight initial predictions will be made.
In similar fashion to the full model, each oxide also has a set of corresponding PLS model, referred to as the \textit{submodels} and each submodel will be specialized to make predictions based on the concentration of a given oxide as predicted by the full model.
...
If the full model predicts a concentration that falls within low-mid or mid-high, the blending approach described in Section~\ref{sec:pls_submodels} is used to combine the predictions from the corresponding submodels.

The training of the models follows the same approach as described in \citet{andersonImprovedAccuracyQuantitative2017}.
...
Furthermore, we have decided to automate outlier removal as described in Section~\ref{methodology_outlier_removal} rather than performing it manually.
...
% TODO: Describe training of PLS-SM:
% Describe k-fold
% Describe train test split
% (Maybe) describe influence plots
% Explain prediction setup - including outlier removal process
% Hyperparameters for PLS - Same as authors
...
The ICA phase in our pipeline can be seen in figure \ref{fig:ica_data}.
