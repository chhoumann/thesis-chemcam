...
% Additionally, by experimenting with alternative methods, we can gain a better understanding of which components contribute the most to the overall error and therefore would benefit the most from further research and development.
...
For outlier removal, there are a few key experiments to perform.

- no outlier removal in PLS1-SM (why? because a significant effort has gone into collecting the calibration dataset with as few outliers as possible)
- add outlier removal to ICA and try a few different approaches (we know they used MAD, but we don't know how they used it - try and improve the performance using MAD)
- try different significance levels for the chi-squared test (make it even more conservative? or less conservative?)
- try alternative outlier removal methods (e.g. Isolation Forest, Local Outlier Factor, etc.)
