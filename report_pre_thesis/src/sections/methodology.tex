...
Firstly, it would be interesting to see how the performance is affected by omitting outlier removal altogether.
examining the performance implications of completely omitting outlier removal would be worthwhile.
This experiment is justified given the substantial efforts dedicated to developing the ChemCam calibration dataset, which implies a minimal presence of significant outliers.Furthermore, experimenting with various significance levels for the chi-squared test could reveal whether a more or less conservative approach is advantageous.
...
However, implementing outlier removal using MAD would still be a compelling experiment to see if it improves the performance of the pipeline.
