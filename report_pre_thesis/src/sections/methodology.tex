...
Afterwards, we mask the edges of the spectral regions of the data to eliminate increased noise, especially in areas where small intensity changes lead to large variations in the calibrated signal.
...
An example of these masked regions for the \texttt{cadillac} sample can be seen in figure \ref{fig:masked_regions}.

\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{images/masked_regions.png}
	\caption{The masked regions of the spectral data for the \texttt{cadillac} sample.}
...
The core concept being that a PLS model, referred to as the \texttt{full model}, will be responsible for making an initial prediction for each oxide and in this case, eight initial predictions will be made. 
In similar fashion to the full model, each oxide also has a set of corresponding PLS model, referred to as the \texttt{submodels} and each submodel will be specialized to make predictions based on the concentration of a given oxide as predicted by the full model.
The submodels are specialized 
