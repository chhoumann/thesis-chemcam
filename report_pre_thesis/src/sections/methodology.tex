...
The fact that we are able to perform ICA with the same algorithm as \citeauthor{cleggRecalibrationMarsScience2017} indicates that our recreation is accurate enough for our purposes of identifying the components that contribute the most to the overall error.
...
%While both PLS1-SM and ICA use the laboratory training set, each technique uses different preprocessing steps to enable comparison between laboratory and Mars data, and approaches the fundamental challenge of quantitative LIBS regression differently. A simple average of both techniques might seem like the best approach. However, after reviewing the geochemical results, certain elements appeared better suited to one technique or the other. This is probably most apparent for aluminum, where the ICA RMSE (4.77 wt.%) of the training set was significantly higher than for PLS1-SM (2.97 wt.%). The ICA method also appears to under-estimate relatively high values of Al2O3 as depicted in Fig. 4. The solution selected for Al2O3 is to ramp progressively the proportion from 0/100 PLS1/ICA to 75/25 PLS1/ICA. To achieve this result, a preliminary estimate is made with 75/25 PLS1/ICA, and if the result is strictly b 15 wt.%, then the PLS1 proportion is increased linearly from 0 to 75 as a function of the preliminary estimate. Still, the final test set trends toward a lower Al2O3 composition at higher certificate values as depicted in Fig. 4 with a 0.83 r2. In spite of that, the resulting Al2O3 Mars values appear well suited toward reality based on comparison with known mineral abundances and Alpha Particle Xray Spectrometer (APXS) comparisons
