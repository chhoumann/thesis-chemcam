...
As previously mentioned in Section~\ref{sec:introduction}, the PLS aspect of the pipeline follows a submodel approach to make oxide predictions.
A PLS model is trained on all the data to make an initial prediction for each oxide.
This model is referred to as the \textit{full model}.
Based on the full model's initial predictions, each prediction is then "binned" into one of five concentration ranges.
For each of these five concentration ranges, there is a corresponding \textit{submodel} for each oxide.
