...
However, not all details were included in \citet{cleggRecalibrationMarsScience2017}, so we have had to make some assumptions, which we detail here.
...
These regions are defined in \citet{cleggRecalibrationMarsScience2017} 40.811 - 246.635, 338.457 - 340.797, 382.138 - 387.859, 473.184 - 492.427, and 849 - 905.574 nm.
...
The preprocessing steps for the ICA phase are illustrated in figure \ref{fig:ica_data}.
...
% TODO: Mention that we only use one of the five datasets for each target for now because of missing information about how each of the five datasets are used for each target for ICA.
...
However, we do not weigh by the inverse of the instrument response function (IRF) as mentioned in \citet{cleggRecalibrationMarsScience2017} before normalizing.
