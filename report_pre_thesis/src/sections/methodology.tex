...
For the PLS1-SM phase, the 50 laser shots for each location are averaged together, resulting in 5 spectra per target.


% Each target was analyzed using 50 laser shots in 5 separate locations.
% The 50 spectra for each location are averaged together, resulting in 5 spectra per target.
...
After the shared preprocessing, we apply two different normalization techniques, Norm1 and Norm3, to the data.
...
These steps are identical to the original pipeline described by \citeauthor{cleggRecalibrationMarsScience2017} and are illustrated in figure \ref{fig:ica_data}.
