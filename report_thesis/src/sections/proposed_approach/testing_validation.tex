...
Secondly, this trade-off ensures a more stable and reliable assessment of the model's performance on typical data points.
...
Although the test set may be less representative of the full range of data, particularly concerning the rare extreme values, the evaluation focuses on the model's ability to generalize from the training data to new data within the same distribution range.
Essentially, this trade-off ensures that the model is evaluated on data points within the range the model was trained on, which is a more realistic evaluation of the model's performance.
By using cross-validation in conjunction with a separate test set, we ensure that the model is both robust and performs well under typical conditions.
...
Metrics obtained from cross-validation provide a broader view of the model's accuracy and robustness across the entire dataset, including extreme values.
...
Therefore, it is crucial to evaluate the model's performance using both the cross-validation metrics (\texttt{rmse\_cv} and \texttt{std\_dev\_cv}) and the test set metrics (\texttt{rmsep} and \texttt{std\_dev}).
