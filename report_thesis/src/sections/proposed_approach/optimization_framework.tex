...
% The first criterion was that the framework should be able to search through a combined search space of models and preprocessing techniques.
% This means that the number variables to search through is large. 
% Therefore, traditional methods such as grid search or random search are not feasible, as they would most likely require an infeasible amount of time to correctly identify the optimal pipelines for each oxide.
% This leads to the next criteria, which is an efficient search strategy and early stopping.

% So: We're doing the search.
% We need to ensure that this search aligns with our goals defined in problem def. Need models that generalize, are robust, and are accurate. (callback: problem def). Can't be a random search due to the amount of variables to check. Likewise for grid-search. Would take too long for our task.
% The search should have an objective function that seeks to achieve these goals.
% We have established our process for determining whether models (configurations!) are robust & accurate based on the metrics we define. (callback: testing-validation). This includes both CV & eval on a test set.
% We emphasize the importance of evaluating performance based on cross validation metrics. As mentioned in the testing-validation section, searching for RMSEP will lead to misleading & poor results!
% So we define the objective function we wish to optimize as a Multi-objective Optimization on minimizing the rmse_cv and std_dev_cv metrics.
