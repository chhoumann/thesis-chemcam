...
The function responsible for running and managing the optimization process, seen in Algorithm~\ref{alg:study_function}(\nameref{alg:study_function}), and the function containing the optimization logic, seen in Algorithm~\ref{alg:combined_objective}(\nameref{alg:combined_objective}).
...
In Line~\ref{step:initialize_sampler} we initialize the sampler with a set of parameters.
In practice, there are several options for the sampler, which means that the choice of sampler parameters is dependent on which sampler is chosen.
