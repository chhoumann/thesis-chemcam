...
The purpose of the \nameref{alg:study_function} is to ensure that the optimization process is done for one model at a time, for each oxide.
...
To manage the optimization process the function receives the number of trials to run, a list of models, and a list of oxides, seen in line~\ref{step:initialize_run_process}, and initializes the sampler, seen in lines~\ref{step:initialize_sampler}.
...
Guiding the optimization process is the \nameref{alg:combined_objective} function, which evaluates the performance of each trial, seeking to either minimize or maximize some specified metrics.
