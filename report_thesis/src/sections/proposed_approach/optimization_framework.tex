...
% Role of optimizer to minimize rmsecv and std dev. Should be written instead
% Move trail loop below model loop X
% Rename second algo to objective which is what is called then
% Support oxide in objective
% Figure out how to make the optimizer play with objective in code
...
\For{each oxide $o$ in $O$} \label{step:oxide_loop}
    \For{each model $m$ in $M$} \label{step:model_loop}
        \For{each trial $t$ from 1 to $N$} \label{step:trial_loop}
            \State \texttt{optimize}(\texttt{objective}(
...
\begin{algorithmic}[1]
\Require Model $m$, Oxide $o$, Sampler \texttt{sampler}, Pruner \texttt{pruner} \label{step:combined_objective_params}
...
\Statex
\State $T_{cv}, D_{train}, D_{test} \gets \text{apply pipeline to } T_{cv}, D_{train}, D_{test}$ \label{step:apply_pipeline_train}
\Statex
