...
Figure \ref{fig:histogram_grid_plot} presents histograms and \gls{kde} curves for \ce{SiO_2} concentrations in each of the four cross-validation folds and the full test set.
...
The legends detail the fold number and respective counts of unique targets and total samples.
As can be seen, the consistent distributions across folds suggest effective data partitioning, ensuring each fold is representative of the overall dataset.

The consistency observed across the folds indicates that our cross-validation method effectively captures the underlying distribution of \ce{SiO_2} concentrations.
This balance is crucial for developing generalizable models, as it ensures that each fold contains a similar distribution of \ce{SiO_2} values.
The \gls{kde} curves' alignment across folds confirms the robustness of our custom k-fold cross-validation procedure in maintaining consistent data distribution.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=\textwidth]{images/histogram_kde_plot.png}
    \caption{Combined Histogram and \gls{kde}\ce{SiO_2} Distribution in Each Fold.}
...
This comprehensive visualization enables direct comparison of the distributions across the folds and the test set.
The x-axis shows \ce{SiO_2} concentration, while the y-axis indicates the count of samples per bin.
The alignment of \gls{kde} curves across folds and the test set suggests that our custom k-fold cross-validation method effectively maintains representativeness and balance in the data.
...
We conclude that the consistency observed across folds supports that the data distribution method works as intended, providing confidence in the generalizability and reliability of our models.
