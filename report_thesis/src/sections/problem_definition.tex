...
The objective is to develop a computational model, denoted as the mapping function $\mathcal{F}: \mathbb{R}^N \rightarrow \mathbb{R}^{n_o}$, to predict the concentrations of major oxides in geological samples from processed \gls{libs} spectral data. The model will utilize feature vectors $\mathbf{x} \in \mathbb{R}^N$ derived from the Masked Intensity Matrix $\mathbf{M}[\chi, l, \lambda]$ and output Estimated Concentration Vectors $\mathbf{v} \in \mathbb{R}^{n_o}$. 
