...
Additionally, we will evaluate the robustness of our models using the standard deviation of prediction errors, defined as:
\[
\sigma_{error} = \sqrt{\frac{1}{n-1} \sum_{i=1}^{n} (e_i - \bar{e})^2}
\]
where \( e_i = \mathbf{v}_i - \hat{\mathbf{v}}_i \) and \( \bar{e} \) is the mean error. This metric assesses the variability of the prediction errors, offering insights into the consistency and reliability of the model's performance. A lower standard deviation indicates a more robust model.
...
The cleaned data is used as input to machine learning models, which have been trained to correlate specific spectral signatures with known concentrations of elements, based on a database of Earth-based and synthetic Martian analogs.
