...
Additionally, to demonstrate the potential for further performance enhancement, we experimented with stacking ensemble methods by using the developed catalog of models and investigated the impact of using a different meta learner for each oxide.
While these initial experiments indicate promising results, we did not fully explore all possible combinations of models and preprocessing techniques due to time constraints.
This suggests a fruitful area for future research to optimize and expand upon our findings.
These promising results highlight potential for future research to optimize and expand model and preprocessing technique combinations.

% Instead of mentioning that we have superior performance, we should instead focus on the fact that we have a catalog of models and preprocessing techniques that can be used to predict major oxide compositions in LIBS data. And this is what we should mention above instead.
% Then, we can mention that one could further improve the performance by using stacking ensemble methods to create a meta learner for each oxide, which we briefly experiment with in this work too in order to demonstrate the idea, however we do not have the time to fully explore this by testing all possible combinations of models and preprocessing techniques.
