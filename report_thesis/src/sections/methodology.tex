...
While the \gls{ccs} data is in a more suitable form for quantitative analysis, it still requires further preprocessing. Table~\ref{tab:ccs_data_example} shows an example of the \gls{ccs} data for a single target location of a sample. This corresponds to shots ($s$) and wavelength ($\lambda$) of the intensity matrix for this location.
...
The remaining 45 shots from each location are then averaged, yielding a single spectrum per location and resulting in a total of five spectra for each target, as represented by the Averaged Intensity Matrix . 
