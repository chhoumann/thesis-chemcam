...
As described in Section~\ref{subsec:stacked-generalization}, is a method that utilizes multiple base estimators trained on the same data, whose predictions are then used to train a meta-learner.
...
The following table summarizes the preprocessing techniques and models selected for our experimentation:

\begin{table}[ht]
\centering
\begin{tabularx}{\columnwidth}{lX} % Define the columns, one fixed-width and one flexible
\toprule
\multirow{5}{*}{Preprocessing Techniques:} & Z-Score Normalization \\
                                           & Min-Max Normalization \\
                                           & Max Absolute Scaling \\
                                           & Robust Scaling \\
                                           & Norm 3 \\
\midrule
\multirow{6}{*}{Models:}                   & Partial Least Squares \\
