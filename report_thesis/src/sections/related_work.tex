The analysis of LIBS data has evolved significantly with various machine learning and statistical methods being employed to address the inherent challenges. This section summarizes key research efforts, organized by thematic areas relevant to our study.

Machine learning models have become a cornerstone in the analysis of LIBS data, particularly for quantifying major oxides on Mars. 
\citet{andersonPostlandingMajorElement2022} explored a variety of models, including \gls{gbr} and \gls{rf}, for the SuperCam instrument on the Perseverance rover. 
Their study highlighted the strengths of \gls{pls} for longer distances and \gls{gbr} for closer ranges. 
These findings demonstrate the effectiveness of different machine learning models in handling the variability of LIBS data and showcasing the adaptability and precision of these models.

Handling the high dimensionality of LIBS data is a major challenge. 
\citet{rezaei_dimensionality_reduction} explored various statistical and machine learning methods, including \gls{mlr}, \gls{svr}, \gls{ksvr}, and \gls{ann}. 
They integrated these models with \gls{pca} to reduce dimensionality and improve performance. Their results demonstrated that dimensionality reduction techniques significantly enhance the predictive capabilities of machine learning models. 
Notably, \gls{ksvr} performed particularly well when combined with \gls{pca}, highlighting the importance of dimensionality reduction in managing the complex, high-dimensional nature of LIBS data.

Incorporating domain knowledge into machine learning models can improve both interpretability and performance. 
\citet{song_df_k_elm} presented a novel approach using knowledge-based spectral lines to construct a linear physical principle-based model, augmented with \gls{k_elm} to account for residuals. 
This \gls{df}-\gls{k-elm} method was tested across multiple regression tasks and showed superior performance, highlighting the benefits of integrating domain-specific insights into model development. 
This approach is particularly relevant as it addresses the interpretability challenges of complex machine learning models.
