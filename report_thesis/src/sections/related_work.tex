...
\citet{mulenga_stacking_2021} introduce novel techniques for improving the preprocessing of microbiome data. 
The authors present a unique approach that involves stacking and chaining multiple normalization methods to optimize the preprocessing pipeline for deep learning applications in colorectal cancer detection. 
Their stacking method combines the outputs of various normalization techniques to artificially expand the training dataset, whereas the chaining method applies these techniques sequentially to incrementally adjust data towards a normal distribution.
The effectiveness of these methods was evaluated using area under the curve (AUC) metrics, with results showing significant improvements in model performance, achieving AUC values between 0.857 and 0.987. 
This suggests that careful selection and combination of preprocessing methods can substantially enhance the predictive accuracy and robustness of models dealing with complex, high-dimensional data.
This approach is interesting to our study as it offers a potential pathway to address similar challenges in LIBS data analysis, such as high dimensionality and multicollinearity, which significantly affect the accuracy of predicting major oxide compositions. 
Integrating a tailored stacking and chaining normalization framework into our preprocessing strategy could improve the robustness and accuracy of our models, aligning with our objectives to enhance the prediction capabilities of major oxides from LIBS data. 
