...
The main idea behind \gls{kernel-pca} is to extract the principal components of the data, similar to \gls{pca} as described in Section~\ref{subsec:pca}.
However, unlike \gls{pca}, \gls{kernel-pca} does not compute the covariance matrix of the data directly.
This is due to the fact that the covariance matrix is often infeasible to compute for high-dimensional datasets.
