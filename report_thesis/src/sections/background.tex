...
As shown in figure \ref{fig:moc_pipeline}, the input to the \gls{moc} model is \gls{ccs} data.
This spectral data is collected on Earth in a laboratory setting simulating the Martian environment.
For each sample, five \gls{ccs} datasets are collected by shooting 50 times at the sample from different locations.
As a result, the total feature count per sample amounts to 6144 (2048 channels per spectrometer $\times$ 3 spectrometers).
Consequently, the \gls{ccs} data for each sample forms a substantial high-dimensional matrix with dimensions $50 \times 6144$.
Complementing the data is a matrix of the corresponding major oxide concentrations for each sample, which serves as the target variable for the model.
