...
Min-Max normalization rescales the range of features to $[0, 1]$ or $[a, b]$. 
The goal is to normalize the range of the data to a specific scale, typically 0 to 1. The formula for Min-Max normalization is:

$$ 
	v' = \frac{v - \min(F)}{\max(F) - \min(F)} \times (new max - new min) + new min
$$

where $v$ is the original value, $\min(F)$ and $\max(F)$ are the minimum and maximum values of the feature $F$ respectively, and $new min$ and $new max$ define the range of the transformed data. 
This type of normalization is particularly useful when algorithms such as neural networks are sensitive to the magnitude of input features and require normalization to ensure more effective and faster convergence~\cite{dataminingConcepts}.
