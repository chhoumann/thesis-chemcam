...
The objective function to be minimized is composed of a loss function $L$ that measures the difference between the prediction $\hat{y}_i$ and the target $y_i$ for each sample $i$.
The objective function is defined as:

$$
\mathcal{L}(\phi) = \sum_{i=1} L(\hat{y}_i, y_i) + \sum_{k=1} \Omega(f_k),
$$

Where $\Omega$ is the regularization term, and $f_k$ is the $k$-th independent tree.

The core mechanism of \gls{xgboost} can be summarized in the following way:
\begin{enumerate}
	\item The algorithm first finds canditate splitting points according to the percentiles of the feature distribution 
	\item It then maps these features into buckets based on the candidate splitting points
	\item Once split, the algorithm calculates 
\end{enumerate}
