...
The description of min-max normalization is based on \citet{dataminingConcepts}.
Min-max normalization maps a value $v_i \in A$ to $v'_i$ in the range $[a, b]$, where $a$ and $b$ represent the new minimum and maximum values, respectively.
Mathematically, min-max normalization is defined as:
$$
    v' = \frac{v - \min_A}{\max_A - \min_A} \times (b - a) + a,
$$
where $v$ is the original value, $\min_A$ and $\max_A$ are the minimum and maximum values of the feature $A$, respectively.

This type of normalization is beneficial because preserves the relationship found in the original values.
