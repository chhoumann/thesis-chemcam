...
Gradient Boosting is an ensemble learning algorithm that combines multiple weaker learners to predict a target variable.
...
Next, we compute the residuals \(\hat{r}_{1,i}\) for each data point \(i\). These residuals are the gradients of the loss function with respect to the current predictions. Formally, this is expressed as:

$$
\hat{r}_{1,i} = \frac{d \mathcal{L}(y_{i}, \hat{y}_{i})}{d \hat{y}_{i}}
$$

where \(\hat{y}_{i} = F_{1}(x_{i})\). These residuals indicate how much each prediction needs to be adjusted to decrease the loss.
