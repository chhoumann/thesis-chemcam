...
$$
where $L(y,\hat{y})$ is the chosen loss function, which measures the discrepancy between the true values $y$ and the predicted values $\hat{y}$, $N$ is the number of samples, $y_{i}$ are the true values, and $\gamma$ is a constant that represents the prediction of the initial model.
