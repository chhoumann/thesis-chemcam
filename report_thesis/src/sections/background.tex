...
\gls{pca} is a dimensionality reduction technique that transforms a set of possibly correlated variables into a smaller set of uncorrelated variables called \textit{principal components} \cite{James2023AnIS}.
...
Having previously introduced \gls{pca}, we now describe \gls{pls}.
...
First, \gls{pca} is applied to the dataset $\mathbf{X}$, transforming it into a set of uncorrelated variables, the principal components \cite{James2023AnIS}.
...
We therefore provide an overview of \gls{svm}s before discussing \gls{svr}s.

\gls{svm} is a supervised learning algorithm used primarily for classification tasks \cite{James2023AnIS}.
...
The data points that lie closest to the hyperplane are called \textit{support vectors}.

% Mathematically, \gls{svm} solves the optimization problem:
...
% 	\item $\xi_i$ are slack variables that allow some points to be within the margin or misclassified, and
% 	\item $C$ is a regularization parameter balancing the trade-off between maximizing the margin and minimizing the classification error.
% \end{itemize}

\gls{svr} extends the principles of \gls{svm} to regression problems \cite{druckerSVR}.
Instead of finding the hyperplane that best separates data points, \gls{svr} aims to fit a function that predicts continuous values.
But instead of using a hyperplane to separate the data, \gls{svr} uses two parallel hyperplanes to define a margin within which the function should lie, often referred to as the \textit{tube}.
...
\begin{itemize}
	% \item $\mathbf{w} = [\beta_1, \beta_2, \ldots, \beta_n]$ is the weight vector,
	% \item $\mathbf{x} = [x_1, x_2, \ldots, x_n]$ is the input vector, and
	\item $\mathbf{w}$ is the weight vector,
	\item $\mathbf{x}$ is the input vector, and
	\item $b$ is the bias term.
\end{itemize}

The two parallel hyperplanes at a distance $\epsilon$ from the hyperplane are as:
...
% \begin{itemize}
% 	\item $\xi_i$ and $\xi_i^*$ are slack variables that allow for some errors beyond the $\epsilon$ margin, and
% 	\item $C$ is a regularization term that controls the trade-off between the flatness of the function and the tolerance for errors larger than $\epsilon$.
% \end{itemize}
