...
Continuing with the discussion of ensemble learning models, we introduce \gls{gbr} based on Chapter 10.10 of \citet{hastie_elements}.

Gradient Boosting is a powerful machine learning technique used for regression and classification tasks.
The core idea is to build a model from weak learners, typically decision trees, in a sequential manner.
Each subsequent model aims to correct the errors of its predecessor, resulting in a strong predictive model.

\gls{gbr} is a specific implementation of gradient boosting for regression tasks.
\gls{gbr} builds a series of decision trees sequentially, where each tree tries to reduce the errors made by the previous one.
It starts by constructing a model with an initial prediction and then iteratively adds decision trees to improve the model's accuracy.
Each new tree is trained to predict the residuals of the current model.

The loss function, $L(f)$, is minimized during this process.
The loss function quantifies the discrepancy between the predicted values and the actual target values, aiming to reduce this discrepancy as much as possible.
Formally, the loss function can be expressed as:

$$
L(f) = \sum_{i=1}^n L(y_i, f(x_i)),
$$

where $y_i$ is the actual target value, $f(x_i)$ is the predicted value, and $L(y_i, f(x_i))$ measures the error for each instance $i$.

Instead of directly using the gradient of the loss function, \gls{gbr} uses residuals as an approximation.
Residuals are the differences between the actual values and the predicted values from the current model.
A weak learner, often a simple decision tree with limited depth, is fitted to these residuals.
This weak learner tries to capture the errors made by the current model.

The output of the weak learner is used to update the parameters of the model.
Specifically, the model is updated in the direction that reduces the residuals, thereby improving the accuracy of the predictions.
The update can be expressed as:
$$
f_m = f_{m-1} + \rho_m g_m,
$$

where $f_m$ is the model at iteration $m$, $f_{m-1}$ is the model at the previous iteration, $\rho_m$ is the learning rate, and $g_m$ is the output of the weak learner at iteration $m$.
