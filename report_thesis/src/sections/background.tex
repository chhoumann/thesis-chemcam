...
Z-score normalization, also known as standard score normalization, transforms the data into a distribution with a mean of zero and a standard deviation of one. 
This normalization is useful when the actual minimum and maximum of a feature are unknown or when there are outliers that may skew the distribution significantly. 
...
By transforming the data using the Z-score, each value reflects the number of standard deviations away from the mean. 
This method is helpful in environments where the data needs to be standardized to fit a normal distribution, making it easier to compare differences in data points~\cite{dataminingConcepts}.
