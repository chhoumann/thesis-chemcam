...
Stacked generalization, introduced by \citet{wolpertstacked_1992}, is a technique designed to improve the predictive performance of machine learning models by leveraging the strengths of multiple models.

In stacked generalization, multiple base models (level-0 generalizers) are trained on the original dataset. 
The predictions of these base models are then used as inputs to a meta-model (level-1 generalizer), which is trained to make the final prediction. 
This approach allows the meta-model to learn how to best combine the outputs of the base models to minimize generalization error.

To formalize, let $\mathbf{X}$ be the input data and $\mathbf{y}$ be the target variable. 
First, $N$ base models $G_1, G_2, \ldots, G_N$ are trained on the dataset $\mathbf{X}$. 
Each base model produces a set of predictions $\hat{\mathbf{y}}_i = G_i(\mathbf{X})$.
...
The training process of the meta-model typically involves cross-validation to ensure robust performance.
