...
The implicit evaluation of the PLS-SM is interesting since if we did not see a big difference in RMSE between our own and clegg et. al's, then that would call into question of doing outlier removal at all for PLS-SM.

**Table 1**
<br>
As you can see from the table, our outlier removal had almost no efffect on the RMSE. 
This result was in many ways surprising to us. 
We did not necessarily have any expectation that our outlier removal would perform better than clegg et. al's, however, we would have expected to see some effect since we were removing quite a few outliers that theoretically should have a high, negative impact on the model's predictions.  
...
However, given that our results with both types of outlier removal are generally close to the results of Clegg et. al's we determine that the outlier removal component of the PLS-SM phase is not a component that would provide many improvements to the overall RMSE.
...
Initially, one would think that this is not necessarily the most informative experiment since we would just be implementing what should already have been there. 
However, it allowed us to do a with/without outlier removal test, similar to the PLS, and as such test the intrinsic robustness of the ICA model.
...
Recall that in our baseline implementation we used 1 dataset per sample.
...
**Table 4.2**
The previous results then led us to wonder how much of an effect outlier detection would have with this aggregated dataset. Since the signal in data was clearly being obscured, we reasoned that this would also make it easier to pick out any outliers. 
...
Generally, however, we determine that a "one size fits all" approach to outlier detection does not provide sufficient benefits and therefore specialty approaches needs to be developed. Perhaps this involves developing approaches that cater more specifically to each oxide.
