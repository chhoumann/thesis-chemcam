...
::: {.notes}
In the 1970s, the NASA Viking missions were the first to land on Mars.<br>
...
We have worked with one of the authors of these papers, Jens Frydenvang, who asserted that the current model has room for improvement, especially in its accuracy and robustness.<br>
In the context of this project, accuracy means how well the model's predictions match reality, while robustness refers to how robust the model is to the variations we see on Mars, since the model is trained on samples from Earth.<br>
...
Our goal was to replicate the MOC model and perform experiments to identify the components that contribute the most to the overall error $E(M)$ (accuracy), as defined by the root mean squared error (RMSE).
...
Firstly, our samples often contain additional elements beyond the major oxides we focus on, which can result in a total below 100%.<br>
